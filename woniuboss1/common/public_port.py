...
class WoniuBoss:
   ...
   def post_login_method(self):
        ...
        response=self.session.get('http://106.13.36.122:8080/WoniuBoss2.5/log/userLogin?userName=WNCD000&userPass=woniu123&checkcode=0000')
        # vp={'vp':'woniu123'}
        # response=self.session.post('http://wf:8080/WoniuBoss2.0/second?vp=woniu123',vp)
        #
        return response
   def post_login_method1(self):
        #login_data={"userName":"WNCD001","userPass":"woniu123","checkcode":"0000"}
        response = self.session.get(
            'http://106.13.36.122:8080/WoniuBoss2.5/log/userLogin?userName=WNCD000&userPass=woniu123&checkcode=0000')
        return response
   ...
   def decode_method(self):
       vp={'vp':'woniu123'}
       response=self.session.post('http://106.13.36.122:8080/WoniuBoss2.5/second?vp=woniu123',vp)
       return response
   def post(self,url,data,headers=None,file=None):
       return self.session.post(url,data,headers=headers,files=file)
