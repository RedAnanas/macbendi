...
from common.public_port import WoniuBoss # 调用接口公共方法
from common.Utility import * # 调用读取execl方法
...
class Trainning_resourses(unittest.TestCase):
        ...
        for i in range(1, 4):## sheet.nrows
            ...
            try:
                ...
                if data['cusInfo']=='郝保成':## 用例1
                    self.assertEqual(1,resp.json()['totalRow'])  # 期望值正确：1
                elif data['cusInfo']=='吴帆':## 用例2
                    self.assertEqual(0,resp.json()['totalRow'])
                else:## 用例3
                    self.assertEqual(23,resp.json()['totalRow']) ##OK
    ...
    def test_Trainning_resourses_public(self):
        ...
        for i in range(7, 9):## sheet.nrows
            ...
            try:
                ...
                # 对响应进行断言
                if data['cusInfo']=='':
                    self.assertEqual(resp.json()['totalRow'],7)
    ...
    def test_Trainning_resourses_givePeople(self):
        ...
        self.assertEqual(200, resp.status_code)
        # 对响应进行断言
        self.assertEqual(30,resp.json()['totalRow'])
